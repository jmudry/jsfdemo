...
public class PersonFormBean implements Serializable {
	...
	public void validatePinDob(ComponentSystemEvent event) {
		...
		UIInput dob = (UIInput) form.findComponent("dateOfBirth");
	...
	//sprawdza czy istnieje już taka osoba o takim imieniu
	public void validatePerson(FacesContext context, UIComponent compoment, Object value){
		for(Person person : pm.getAllPersons()){
				String imie = (String) value;
				if (imie.equalsIgnoreCase(person.getFirstName())) throw new ValidatorException(new FacesMessage("Osoba taka już istnieje w bazie"));
			}	
	}
